...
app.get("/packages/:packages", (request, response) => {
  try {
    ...
    packages.forEach((pkg) => {
      log.info(`Processing: ${pkg.name} package`);
      const regex = new RegExp(`[\\W]${pkg.name}$`);
      let exists =
        fs.existsSync(`./node_modules/${pkg.name}`) ||
        (() =>
          process.moduleLoadList.forEach((m) => {
            if (regex.exec(m)) return true;
          }))();
      log.info(`${pkg.name} package ${exists ? "already" : "no"} exists into app-repository server.`);

      if (!exists) {
        log.info(`Executing npm install of the ${pkg.fullName}...`);
        exec(`npm install ${pkg.fullName} --verbose`, (error, stdout, stderr) => {
      ...
      log.info(`Browserifing package: ${pkg.name}`);
      browserify.require(pkg.name);
    });
